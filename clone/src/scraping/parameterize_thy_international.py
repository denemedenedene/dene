...
class Params:
    ...
    parser.add_argument("--OWRT",help="One Way/Round Trip info", \
                        choices=["OW","RT"])
    parser.add_argument("--ss",help="Enter amy value to take screenshot while scraping")
...
element = wait.until(EC.presence_of_element_located(('xpath','//*[contains(@src,"fareDrivenNew.tk")]')))
if flight.ssInfo != None:
    driver.save_screenshot('THYi_1.png')
    print('ss1 saved')

driver.switch_to_frame(driver.find_element_by_tag_name("iframe"))

if flight.arrAirport != "JFK" and flight.arrAirport != "IAD" and flight.arrAirport != "SFO": #for EU flights
        ...
        for i in range(0,12):   
            try:
                if driver.find_element_by_xpath('//*[@id="b0Flight{}Segment0TrEl"]/th[1]/span[1]/time'.format(i)).text == flight.cnvDepTime:
                    driver.find_element_by_xpath('//*[@id="b0FO{}_TREUPROTdEl"]'.format(i)).click()
                    print('clicked from try and i=',i)
                    break
                else:
                    print('tıklayamadık yauv :(')
            except:
                print('first try - inner except')      
                break
        ...
        for i in range(0,12):
            try:
                ...
                if driver.find_element_by_xpath('//*[@id="table-fly_list1"]/tbody[{}]/tr/td[1]'.format(i)).text == flight.cnvDepTime:
                    ...
                    print('clicked from except')
                    break
            except:
                print('outer except inner except i mean except except lol')
                break    
else:#for murica flights
    try: ## new flight picker
        for i in range(0,12):   
            try:
                if driver.find_element_by_xpath('//*[@id="b0Flight{}Segment0TrEl"]/th[1]/span[1]/time'.format(i)).text == flight.cnvDepTime:
                    driver.find_element_by_xpath('//*[@id="b0FO{}_TRUSEF2TdEl"]'.format(i)).click()
                    print('clicked from try')
                    break
                else:
                    print('tıklayamadık yauv :(')
            except:
                print('second try - inner except')
                break          
...
if flight.ssInfo != None:
    driver.save_screenshot('THYi_2.png')
    print('ss2 saved')
...
if flight.ssInfo != None:
    driver.save_screenshot('THYi_3.png')
    print('ss3 saved')
